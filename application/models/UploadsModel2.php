...
class UploadsModel2 extends CI_Model
{

	private $t1 = 'document_upload_sewaktu';
	private $_t1 = 'v_document_upload_sewaktu';
	private $t2 = 'document_table_sewaktu';
	private $t4 = 'dk_kategori';

	var $_t1_select_column = array("id","id_document",	"document_name", "status_dokumen","nama_status", "keterangan","tanggapan","nama_intansi","jenis", "user_id", "created", "updated","status_dokumen2","id_puskesmas");
	var $_t1_order_column = array("id","id_document",	"document_name", "status_dokumen","nama_status", "keterangan","tanggapan","nama_intansi","jenis", "user_id", "created", "updated","status_dokumen2","id_puskesmas");
	...
	{
		$sql="SELECT * FROM v_document_upload_sewaktu WHERE status_dokumen !=? and id_puskesmas=? ORDER BY updated desc LIMIT 30";
	...
	public function do_ajukan($id=null)
	{
		...
		if($cekdata->num_rows()!=0){
			...
			if ($hasil) {
				return $arrayName = array('pesan' => 'Proses pengiriman berkas ke Dinas berhasil', 'status' => 1);
			} else {
				return $arrayName = array('pesan' => 'Proses pengiriman berkas ke Dinas gagal', 'status' => 0);
			}
		}else{
			return $arrayName = array('pesan' => 'tidak ada file yang akan dikirimkan, periksa kembali draft ajuan', 'status' => 0);
		}		
	}
	...
	{
		$check_where = array('document_name' => $data['dokument_name']);
		...
		if ($cek_data) {
			if ($cek_data->num_rows() >= 1) {
				...
				$data_dokumen = array(
					...
					'status_dokumen' => $data['status_dokumen'], 'keterangan' => $data['keterangan'],
	...
	public function saveFilePerbaikan($data = null)
	{
		...
		if ($hasil) {
			/*proses hapus file */
				$path = FCPATH . 'dokumentsewaktu/' . $data['file_lama'];
				if(file_exists($path)){
					unlink($path);
	...
	public function deleteFileById($id = '')
	{
		...
		if ($check->num_rows() >= 1) {
			...
			$path = FCPATH . 'dokumentsewaktu/' . $data_delete->file_name;
			if(file_exists($path)){
				unlink($path);
			}
	...
	public function deleteFileByIdDocument($id = '')
	{
		...
		if ($check->num_rows() >= 1) {
			...
			foreach ($data_delete as $key) {
				...
				$path = FCPATH . 'dokumentsewaktu/' . $key->file_name;
				unlink($path);
