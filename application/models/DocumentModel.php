...
class DocumentModel extends CI_Model
{
	...
	private $_document_upload = 'document_upload';
	private $_v_document_upload = 'v_document_upload';
	private $_document_table = 'document_table';
	private $t4 = 'dk_kategori';
	private $t5 = 'tb_indikator_penilaian';

	var $_t1_select_column = array("id","id_document",	"document_name",	"id_kategori",	"nama_kategori",	"nama_kelompok",	"duedate",	"status_duedate",	"status_dudate2",	"bulan", "tahun", "status_dokumen", "keterangan", "user_id", "tgl_upload", "created", "updated","status_dokumen2","id_puskesmas","nama_puskesmas","nama_bulan");
	var $_t1_order_column = array("document_name",	"nama_kategori",	"nama_kelompok",	"duedate",	"status_duedate",	"status_dudate2",	"bulan", "tahun", "status_dokumen", "keterangan", "tgl_upload","status_dokumen2","nama_bulan");
	...
	{
		$query='select * from document_table where id_document=?';
	...
	{
		$query='select * from document_table where id=?';
	...
	{
		$hasil = $this->db->query('UPDATE document_upload SET status_dokumen=?, tanggapan=?,updatedby=? WHERE id=?',array($data['status_dokumen'],$data['tanggapan'],$data['updatedby'],$data['id']));
		if ($hasil) {
			return $arrayName = array('pesan' => 'Data dokumen berhasil diupdate', 'status' => 1);
		} else {
	...
	{
		$hasil = $this->db->query("UPDATE document_table SET deskripsi=?,updatedby=? WHERE id=?",array($data['catatan_perbaikan'],$data['updatedby'],$data['id_dokumenya']));
		if ($hasil) {
	...
	{
		$this->db->query('delete from ia_document_review where id_document=?',array($id));
		if ($hasil) {
		...
		if (isset($_POST['search']['value'])) {
  		...
  		if (isset($_POST['filter_tahun'], $_POST['filter_bulan'], $_POST['filter_dokumen'],$_POST['filter_status_dokumen'],$_POST['filter_puskesmas'])) {
            if ($_POST['filter_tahun'] != null && $_POST['filter_bulan'] == null && $_POST['filter_dokumen'] == null && $_POST['filter_status_dokumen'] == null && $_POST['filter_puskesmas']==null) {
				...
				$this->db->where('bulan', $_POST['filter_bulan']);
				$this->db->where('document_name', $_POST['filter_dokumen']);
				$this->db->where('status_duedate', $_POST['filter_status_dokumen']);
            }elseif ($_POST['filter_tahun'] != null && $_POST['filter_bulan'] != null && $_POST['filter_dokumen'] != null && $_POST['filter_status_dokumen'] != null && $_POST['filter_puskesmas']!=null) {
                $this->db->where('tahun', $_POST['filter_tahun']);
				$this->db->where('bulan', $_POST['filter_bulan']);
				$this->db->where('document_name', $_POST['filter_dokumen']);
