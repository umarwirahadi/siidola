...
class Uploadbidangmodel extends CI_Model
{
	...
	var $_t1_select_column = array("id","id_document",	"document_name",	"id_kategori",	"nama_kategori",	"nama_kelompok",	"duedate",	"status_duedate",	"status_dudate2",	"bulan", "tahun", "status_dokumen", "keterangan", "user_id", "tgl_upload", "created", "updated","status_dokumen2","id_intansi","nama_puskesmas","tanggapan");
	...
	public function show($idintansi = '')
	{
		$this->db->order_by('created','desc');
		$data = $this->db->get_where($this->_t1, ['status_dokumen' => 1, 'id_intansi' => $idintansi]);
		if ($data->num_rows() >= 1) {
			return $data->result();
		} else {
			return false;
	...
	public function showAjuan($idintansi='')
	{
		$sql="SELECT * FROM v_document_bidang WHERE (status_dokumen !=? and status_dokumen !=?) and id_intansi=? ORDER BY updated desc LIMIT 30";
		$data=$this->db->query($sql,array(1,3,$idintansi));
		if ($data->num_rows() >= 1) {
			return $data->result();
		} else {
			return false;
	...
	public function showdisetujuimodel($idintansi = null)
	{
		$sql="SELECT * FROM v_document_bidang WHERE status_dokumen =? and id_intansi=? ORDER BY updated desc LIMIT 30";
		$data=$this->db->query($sql,array(3,$idintansi));
		if ($data->num_rows() >= 1) {
			return $data->result();
		} else {
			return false;
	...
	{
		$sql = "SELECT MAX(id) as id from document_upload_bidang LIMIT 1";
	...
	{
		$check_where = array('id_jenis' => $data['id_jenis'],'bulan' => $data['bulan'], 'tahun' => $data['tahun']);
		...
		if ($cek_data) {
			if ($cek_data->num_rows() >= 1) {
				...
				// 	'user_id' => $data['user_id'],'id_intansi'=>$data['id_intansi'],'updatedby'=>$this->session->userdata('user_id'));
				$hasil = $this->db->insert($this->t1, $data);
	...
	public function saveFilePerbaikan($data = null)
	{
		...
		if ($hasil) {
			/*proses hapus file */
				$path = FCPATH . 'dokumentbidang/' . $data['file_lama'];
				if(file_exists($path)){
					unlink($path);
	...
	{
		$sql="UPDATE document_upload_bidang SET keterangan=?,status_dokumen=? WHERE id=? LIMIT 1";
		$data_perbaikan=$this->db->query($sql,array($data['keterangan'],$data['status_dokumen'],$data['id']));
		if ($data_perbaikan) {
			return $arrayName = array('pesan' => 'Dokumen berhasil diupdate coyyy', 'status' => 1);
		} else {
	...
	public function make_query_ajuan()
	{
		...
		$id_pkm 	=$this->session->userdata('id_intansi');
		$this->db->select($this->_t1_select_column);
		$this->db->from($this->_t1);				
		if (isset($_POST['search']['value'])) {
			$this->db->where('user_id',$user_id);
			$this->db->where('id_intansi',$id_pkm);
