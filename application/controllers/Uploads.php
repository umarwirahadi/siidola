 ...
 public function showdisetujui()
 {
	...
	public function do_upload()
	{
			...
			// set id unik untuk berkas agar tidak tertukar dengan berkas lain
				...
				$bulann			=$data_dok['bulan']+1;
				$nama_dokumen	=$this->upl->getDocName($data_dok['kategori']);
				$data_dokumen 	= array('id_document' =>$uniqueid,'dokument_name'=>$nama_dokumen->NAMA_KATEGORI,
								'id_kategori'=>$data_dok['kategori'],'duedate'=>$data_dok['originalduedate'],'bulan'=>$bulann,
								'tahun'=>$data_dok['tahun'],'status_dokumen'=>1,'keterangan'=>$data_dok['keterangan'],'user_id'=>$id_user,'id_puskesmas'=>$id_puskesmas);
	...
	public function do_upload_update()
	{
		$temparr=[];
				...
				$id_user 			=$this->session->userdata('user_id');
				$data_dok			=$this->input->post();
				$uniqueid 			=$data_dok['id_document'];
				$id_document_upload =$data_dok['iddok'];
...
public function ajukan()
{
	...
	public function detailajuan()
	{
		...
		$this->load->view('uploads/Detail',$data);
	}
